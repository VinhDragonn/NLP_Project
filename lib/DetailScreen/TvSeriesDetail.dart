...
class _TvSeriesDetailsState extends State<TvSeriesDetails> {
  ...
  String? mlPrediction;
  bool _predictionCalled = false;

  Future<void> _predictSeriesSuccess(Map<String, dynamic> seriesJson) async {
    try {
      print('🎬 Starting ML prediction for TV Series...');
      final svc = MlPredictService();
      
      // Lấy creators (tương đương directors)
      String creators = 'Unknown';
      if (seriesJson['created_by'] != null && (seriesJson['created_by'] as List).isNotEmpty) {
        creators = (seriesJson['created_by'] as List)
      ...
      if (seriesJson['genres'] != null && (seriesJson['genres'] as List).isNotEmpty) {
        genres = (seriesJson['genres'] as List)
            .map((g) => g['name'].toString())
            .join('|');
      }
      
      // Lấy production company
      String productionCompany = 'Unknown';
      if (seriesJson['production_companies'] != null && 
          (seriesJson['production_companies'] as List).isNotEmpty) {
        productionCompany = seriesJson['production_companies'][0]['name'].toString();
      }
      
      // Episode runtime (trung bình)
      final runtime = seriesJson['episode_run_time'] != null && 
                      (seriesJson['episode_run_time'] as List).isNotEmpty
          ? (seriesJson['episode_run_time'][0] as num).toDouble()
          : 45.0;
      
      final releaseYear = seriesJson['first_air_date'] != null && 
                          seriesJson['first_air_date'].toString().length >= 4
          ? double.tryParse(seriesJson['first_air_date'].toString().substring(0, 4)) ?? 2020.0
          : 2020.0;
      
      final audienceRating = (seriesJson['vote_average'] ?? 5.0).toDouble() * 10;
      final tomatometerCount = (seriesJson['vote_count'] ?? 100).toDouble() / 10;
      final audienceCount = (seriesJson['vote_count'] ?? 100).toDouble();
      
      print('📊 Prediction data (TV):');
      ...
      final res = await svc.predict(
        movieTitle: seriesJson['original_name'] ?? seriesJson['name'],
        directors: creators,
        genres: genres,
  ...
  Future<void> tvseriesdetailfunc() async {
    ...
    if (tvseriesdetailresponse.statusCode == 200) {
      ...
      if (!_predictionCalled) {
        ...
        await _predictSeriesSuccess(tvseriesdetaildata);
      }
    } else {}
    ///////////////////////////////////////////tvseries review///////////////////////////////////////////
