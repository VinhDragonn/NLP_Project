...
class _addtofavoriateState extends State<addtofavoriate> {
  ...
  Future<void> checkfavoriate() async {
    try {
      final isFav = await _favoriteService.isFavorite(widget.id.toString());
      setState(() {
        favoriatecolor = isFav ? Colors.red : Colors.white;
        _loading = false;
      });
    } catch (e) {
      setState(() {
  ...
  void _onFavoritePressed() async {
    final poster = widget.Details[0]['poster_path'] ?? widget.Details[0]['backdrop_path'] ?? '';
    setState(() {
      _loading = true;
    });
  ...
  Future<void> addOrRemoveFavorite(
    ...
    try {
      if (customcolor == Colors.white) {
        await _favoriteService.addFavorite(id, movieData);
      ...
      Fluttertoast.showToast(
          msg: "Lỗi khi thao tác với Firestore: $e",
